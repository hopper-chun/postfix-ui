...
import callback from './useValidateCallback'
...
const useValidate = (rules, local_, local2_) => {
  ...
  const validate = () => {
    ...
    for (const key in rules.value) {
      ...
      // 2025-02-07 심각한 버그
      const target1 = _.get(local, key)
      const target2 = _.get(local2, key)
      const target = target1 !== undefined ? target1 : target2
...
const cellPhone = {
  test: (value) => {
    ...
    // const REGEXP_CELL_PHONE = /^(01([0|1|6|7|8|9])-?([0-9]{3,4})-?([0-9]{4})|0505-?([0-9]{3})-?([0-9]{4}))$/
    const REGEXP_CELL_PHONE = /^(01([0|1|6|7|8|9])|050[0-9]?)-?([0-9]{3,4})-?([0-9]{4})$/
...
const zeroCellPhone = {
  test: (value) => {
    ...
    // const REGEXP_CELL_PHONE = /^(0?1([0|1|6|7|8|9])|0?50([0|1|6|7|8|9])-?([0-9]{3,4})-?([0-9]{4})|0505-?([0-9]{3})-?([0-9]{4}))$/
    const REGEXP_CELL_PHONE = /^(0?1([0|1|6|7|8|9])|0?50[0-9]?)-?([0-9]{3,4})-?([0-9]{4})$/
...
const subdomain = {
  test: (value) => {
    ...
    // 서브도메인을 포함한 전체 도메인 정규식

    const REGEXP_DOMAIN = /^(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}$/

    return REGEXP_DOMAIN.test(value)
  },
  error: `올바른 도메인 형식이 아닙니다`,
}
// 국제화 도메인(한글 도메인 등)을 포함한 정규식
// const internationalDomainRegex = /^([\p{L}\p{N}][\p{L}\p{N}-]{0,61}[\p{L}\p{N}]\.)+[\p{L}]{2,}$/u
...
const ipAddress = {
  test: (value) => {
    ...
    const REGEXP_DOMAIN = /^((25[0-5]|2[0-4]\d|1?\d{1,2})(\.|$)){4}$/

    return REGEXP_DOMAIN.test(value)
  },
  error: `올바른 IP주소 형식이 아닙니다`,
}
...
const userId = {
  test: (value) => {
    ...
    const USER_ID = /^[a-z][a-z0-9]{4,}$/

    return USER_ID.test(value)
  },
  error: `올바른 ID 형식이 아닙니다. 소문자와 숫자만 사용하실 수 있습니다.`,
}
...
const password = {
  ...
  error: `올바른 패스워드 형식이 아닙니다. 영문/숫자/특수문자가 1개 이상씩 포함되어야 합니다.`,
}
