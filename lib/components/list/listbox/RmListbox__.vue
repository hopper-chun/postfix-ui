...
// 현재 슬롯 내부 div에 whitespace-pre 넣었는데, 띄어쓰기때문에 줄바꿈되서 그랬음. 만약 줄바꿈이 필요하다면 whitespace-pre를 제거할 것. 단, 그렇게 하면 inline 일 때 가능한 모든 줄바꿈을 경험하게 될 것임...
</script>

<template>
  <div class="rm-listbox">
    <ul class="items-center" :class="[inline ? 'inline-flex' : 'flex', vertical ? 'divide-x divide-gray-200' : 'flex-col divide-y divide-gray-200']">
      <li
        v-for="(item, index) in list"
        :key="item"
        @click="handleSelect(itemValue(item), index)"
        class="flex w-full cursor-pointer"
        :class="[selectedItem && selectedItem === itemValue(item) ? 'bg-[#E6F6FF]' : '']"
      >
        <div class="relative flex-1">
          <div>
            <div :class="[!disabledButton ? 'pr-[52px]' : '', vertical ? (index === 0 ? 'pr-[12px]' : 'px-[12px]') : '']">
